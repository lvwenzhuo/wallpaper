...
public class SPCookieStore implements CookieStore {
    ...
    private void saveCookie(HttpUrl url, Cookie cookie, String cookieToken) {
        ...
        if (cookieMap != null) {
            ...
            List<String> keyList = new ArrayList<>(cookieMap.size());
            Enumeration e = cookieMap.keys();
            while (e.hasMoreElements()) {
                keyList.add((String) e.nextElement());
            }
            prefsWriter.putString(url.host(), TextUtils.join(",", keyList));
        }
