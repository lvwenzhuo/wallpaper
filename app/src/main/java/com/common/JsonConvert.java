    ...
    private T parseParameterizedType(Response response, ParameterizedType type) throws Exception {
        ...
        if (rawType != CommonResponse.class) {
            ...
            if (typeArgument == Void.class) {
                ...
                response.close();
                //noinspection unchecked
                // if (simpleResponse.code == CommonResponse.CODE_TOKEN_INVALID) {
                //   throw new TokenErrorException();
                // } else if (simpleResponse.code == CommonResponse.CODE_OK){
                return (T) simpleResponse.toResponse();
                //  }else{
                //     throw new SimpleNetException(simpleResponse.code,simpleResponse.message);
                // }
                ...
                // } else if (code == CommonResponse.CODE_TOKEN_INVALID) {
                // throw new TokenErrorException();
                // }/* else if (code == 105) {
                // throw new IllegalStateException("用户收取信息已过期");
                // }*/ //else {
                //直接将服务端的错误信息抛出，onError中可以获取
                // throw new SimpleNetException(code,CommonResponse.msg);
                // throw new IllegalStateException("错误代码：" + code + "，错误信息：" + CommonResponse.msg);
                // }
            }
        }
    }
